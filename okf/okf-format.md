...
description: The OKF v0.1 bundle structure okfi reads and how its frontmatter parser behaves.
...
# Frontmatter okfi parses

A concept file opens and closes its frontmatter with a line containing exactly `---`.
Between those fences okfi reads a **flat `key: value` subset** of YAML — enough for OKF,
...
# Conformance okfi assumes of inputs

Per §9, a conformant bundle has a parseable frontmatter block with a non-empty `type`
on every non-reserved `.md`. okfi is a **consumer**: it MUST tolerate broken cross-links,
